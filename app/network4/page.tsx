...
export default function InfluenceNetworkPage3D() {
  ...
  const { nodes, links } = useMemo(() => {
    ...
    const filteredData = influenceData.filter(
      (d: any) =>
        countriesNames.includes(d.a_first) && countriesNames.includes(d.a_second)
    );
    ...
    const rawLinks = filteredData.filter((d: any) => d[yearkey] >= visibleLimit);
    ...
    <main>
      <div className="bg-[#a0a0a0] rounded-lg ml-3 p-1 shadow-lg absolute top-[10px] z-10">
        <span className="flex flex-wrap gap-2 items-center justify-center">
          <select className="p-2 rounded border border-gray-500" onChange={(e) => setYear(e.target.value)}>
          ...
          <select className="p-2 rounded border border-gray-500" onChange={(e) => setTopNum(e.target.value)}>
          ...
          <select className="p-2 rounded border border-gray-500" onChange={(e) => setType(e.target.value)}>
          ...
          <select className="p-2 rounded border border-gray-500" onChange={(e) => setNet(e.target.value)}>
          ...
          <select className="p-2 rounded border border-gray-500" onChange={(e) => setColorStyle(e.target.value)}>
            <option value="influence">Influence Level</option>
            <option value="solid">Solid</option>
            <option value="continent">Continent</option>
          </select>
          <select className="p-2 rounded border border-gray-500" onChange={(e) => setVisibleLimit(Number(e.target.value))}>
      ...
      <div className="relative w-full h-screen">
        <InfluenceNetworkForceParent  allview={type} nodes={nodes} links={links} />
      </div>
    </main>
  );
}
